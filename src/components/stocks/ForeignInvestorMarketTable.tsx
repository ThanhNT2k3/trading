...
interface ForeignInvestorMarketTableProps {
  type?: "buy" | "sell";
  limit?: number;
}
...
function ForeignInvestorMarketTableComponent({ type = "buy", limit }: ForeignInvestorMarketTableProps) {
  ...
  useEffect(() => {
    ...
    const fetch = async () => {
      try {
        setLoading(true);
        const result = await fetchForeignInvestorMarketData(type, abortController.signal);
        if (isMounted) {
    ...
    return (
      <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-white/3">
        <h3 className="mb-4 text-base font-semibold text-gray-800 dark:text-white/90">
          Foreign Investor - {type === "buy" ? "Buying" : "Selling"}
        </h3>
    ...
    <div className="rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-white/3">
      <div className="border-b border-gray-200 px-6 py-4 dark:border-gray-700">
        <h3 className="text-base font-semibold text-gray-800 dark:text-white/90">
          Foreign Investor - {type === "buy" ? "Buying 📈" : "Selling 📉"} (Top 10)
        </h3>
      </div>
            ...
            {data.map((stock, index) => (
              ...
              >
                <td className="px-6 py-4 text-sm text-gray-600 dark:text-gray-400">
                  <div className="flex items-center justify-center w-6 h-6 rounded-full bg-blue-100 text-xs font-bold text-blue-600 dark:bg-blue-900/30 dark:text-blue-400">
                    {index + 1}
                  </div>
                </td>
...
export default memo(ForeignInvestorMarketTableComponent);
